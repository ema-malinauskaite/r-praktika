...
```{r}
apsk_ir_teor_temp_cv <- data.frame()
for (i in 1:5) {
...
train_teor_temp <- duomenys_cv[train,] %>% .[,1]
train_kontaktai <- duomenys_cv[train,] %>% .[,-1]
...
test_teor_temp <- duomenys_cv[test,] %>% .[,1]
test_kontaktai <- duomenys_cv[test,] %>% .[,-1]
...
apsk_ir_teor_temp_cv <- data.frame(rbind(apsk_ir_teor_temp_cv, cbind(data.frame(apsk_temp), test_teor_temp)))
}
apsk_ir_teor_temp_cv
```

R2 skaiciavimas:
```{r}
r2_cv <- apsk_ir_teor_temp_cv %>% 
